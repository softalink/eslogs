...
pub(crate) fn new_stats_row_max(mut field_filters: Vec<Vec<u8>>) -> Result<StatsRowMax, String> {
...
mod tests {
    ...
    fn run(filters: Vec<&str>, blocks: &[Vec<Vec<Field>>]) -> String {
        let sf =
            new_stats_row_max(filters.iter().map(|s| s.as_bytes().to_vec()).collect()).unwrap();
    ...
    fn test_row_max_rejects_missing_and_wildcard_src() {
        ...
        assert!(new_stats_row_max(vec![b"*".to_vec()]).is_err());
    }
