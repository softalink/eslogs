//! Port of Go `strconv`'s `isprint.go` tables + `strconv.IsPrint`.
//!
//! Generated (comment-stripped) from the Go toolchain's `strconv/isprint.go`
//! (BSD-licensed, `go run makeisprint.go`). These compact purpose-built ranges
//! let the port reproduce Go's `strconv.Quote`/`AppendQuote` printable-rune
//! decision exactly, without pulling in the full Unicode category tables.
//!
//! `IS_NOT_PRINT32` entries are stored with 0x10000 already subtracted (Go's
//! table adds it back at lookup); see `is_print`.
...
fn bsearch_u16(s: &[u16], v: u16) -> usize {
...
fn bsearch_u32(s: &[u32], v: u32) -> usize {
    let (mut i, mut j) = (0usize, s.len());
    while i < j {
        let h = i + (j - i) / 2;
        if s[h] < v {
...
/// Port of Go `strconv.IsPrint`: whether `r` is printable by Go's definition
/// (letters, marks, numbers, punctuation, symbols, ASCII space) — the decision
/// `strconv.Quote`/`AppendQuote` use for `\uXXXX` escaping.
pub fn is_print(r: u32) -> bool {
    if r <= 0xFF {
    ...
    let j = bsearch_u16(IS_NOT_PRINT32, rr);
    !(j < IS_NOT_PRINT32.len() && IS_NOT_PRINT32[j] == rr)
}

/// [`is_print`] for a [`char`].
pub fn is_print_char(c: char) -> bool {
    is_print(c as u32)
}
...
mod tests {
    ...
    #[test]
    fn test_is_print_matches_go() {
        // Verified against the Go toolchain's strconv.IsPrint.
        for (r, want) in [
            (0x20u32, true),
            (0x7E, true),
            (0x7F, false),
            ...
            assert_eq!(is_print(r), want, "is_print(0x{r:X})");
        }
    }
    #[test]
    fn test_is_print_full_range_matches_go() {
        // Exhaustive cross-check over 0..=0x10FFFF against the Go toolchain's
        // strconv.IsPrint (count + rolling hash of the printable set).
        let mut count: u64 = 0;
        let mut h: u64 = 0;
        for r in 0u32..=0x10FFFF {
            if is_print(r) {
                count += 1;
                h = h.wrapping_mul(1000003).wrapping_add(r as u64);
            }
        }
        assert_eq!(count, 148998, "printable-count mismatch vs Go");
        assert_eq!(h, 0xf1a25def235489fc, "printable-set hash mismatch vs Go");
    }
}
