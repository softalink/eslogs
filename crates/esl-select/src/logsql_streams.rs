...
pub fn process_streams_request(storage: &Arc<Storage>, req: &Request, w: &mut ResponseWriter) {
    ...
    let streams = match storage.get_streams(&ca.tenant_ids, &ca.q, limit as u64) {
        Ok(v) => v,
        Err(e) => {
...
pub fn process_stream_ids_request(storage: &Arc<Storage>, req: &Request, w: &mut ResponseWriter) {
    ...
    let start_time = Instant::now();
    let stream_ids = match storage.get_stream_ids(&ca.tenant_ids, &ca.q, limit as u64) {
        Ok(v) => v,
        Err(e) => {
...
pub fn process_stream_field_names_request(
    ...
    let start_time = Instant::now();
    let names = match storage.get_stream_field_names(&ca.tenant_ids, &ca.q, filter) {
        Ok(v) => v,
        Err(e) => {
            w.errorf(
                req,
